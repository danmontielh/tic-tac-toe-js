...
const qNumId = (qEl) => { return Number.parseInt(qEl.id[1])  };
...
const takeTurn = (index, letter) => { return grid()[index].innerText = letter };

const oponentChoice = () => { return qNumId(emtyQs()[Math.floor(Math.random() * emtyQs().length )]) };
